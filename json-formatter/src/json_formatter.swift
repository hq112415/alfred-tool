...
let pidFile = "/tmp/json_formatter_\(getuid()).pid"
...
class AppDelegate: NSObject, NSApplicationDelegate, WKScriptMessageHandler {
    ...
    func createAndShowWindow() {
        ...
        if FileManager.default.fileExists(atPath: htmlPath) {
            ...
            webView.loadHTMLString("""
                ...
                <h2>⚠️ 找不到 json_formatter.html</h2>
                <p>请确保 json_formatter.html 与此程序在同一目录</p>
    ...
    func setupMenuBar() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "关于 JSON Formatter", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(withTitle: "隐藏 JSON Formatter", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        ...
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(withTitle: "退出 JSON Formatter", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    func getHTMLPath() -> String {
        ...
        return (executableDir as NSString).appendingPathComponent("json_formatter.html")
    }
